...
{******************************************************************************
  Função     : UserName
  Descrição  : Obter o nome da Usuario logado na rede windows
 ******************************************************************************}
...
function Formatar(Texto: string; TamanhoDesejado: integer; AcrescentarADireita: boolean = true; CaracterAcrescentar: char = ' '): string;
{
   OBJETIVO: Eliminar caracteres inválidos e acrescentar caracteres à esquerda
             ou à direita do texto original para que a string resultante fique com o tamanho desejado
...
{.: Bruno Borba - Desenvolvedor - APS Informática - 05/07/2007 :.}
...
begin
  ...
  Begin
    ...
    begin
      Application.MessageBox('Nome do relatório não pode ser Vazio', 'Informação...', MB_OK);
      Exit;
    end;
    ...
    Begin
      Grid.DataSource.DataSet.Filtered := False;
      Application.MessageBox('Nome de Relatório já existente. Entre com um novo Nome','Informação...', MB_OK);
      Exit;
    end;
...
//      ppR.Template.LoadFromFile;
    ...
    if Confirmacao then
    begin
      if TMens.Quest('Confirma a Alteração do Layout.') then
      begin
        Grid.DataSource.DataSet.Edit;
        ...
        Application.MessageBox('Relatório salvo com sucesso', 'Informação...', MB_OK);
      end;
    end
    else
    begin
    ...
    Application.MessageBox('Não existem Relatórios a serem Alterados','Informação...',  MB_OK);
end;
...
begin
  ...
  Begin
    ...
    Application.MessageBox('Não Existem Relatórios a serem Exportados','Informação...',  MB_OK);
end;
...
//      TClientDataSet(DataSet).ApplyUpdates(0);
    ...
    TMens.Aviso('Relatório Importado com Sucesso.');
  finally
    ListaRel.Free;
  end;
end;
...
begin
  ...
  begin
    Application.MessageBox('Insira o Nome do Relatório','Informação...', MB_OK);
    Exit;
  end;
  ...
  begin
    ...
    begin
      Application.MessageBox('Nome de relatório já existente entre com um Nome diferente', 'Informação...', MB_OK);
      Exit;
    end;
  end;
...
begin
  ...
  {1 Verifica se o Diretorio Existe. Caso não exista, será criado}
  ...
  begin
    try
      Arquivo := TStringList.Create;
      {Carrega com o Arquivo já existente}
      Arquivo.LoadFromFile(Caminho);
...
begin
  //Montar o sql de validação do arredondamento
...
begin
  // Cria o formulário de mensagem se nao foi criado
